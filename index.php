<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Empreste Fácil - Home</title>    
    <link rel="stylesheet" href="CSS/tipografia.css">
    <link rel="stylesheet" href="CSS/home.css">

</head>
<body>
    <main>
        <div class="cont-divisao">
            <div class="cont-bloco">
                <h1>Gerencie seus itens emprestados de forma fácil e dinâmica</h1>
                <p>Uma maneira fácil e dinâmica de gerenciar itens emprestados. Tenha a relação dos seus itens, de quem emprestou e controle a data de devolução.</p>
                <button><a href="cadastro.php">Cadastra-se</a></button>
            </div>
            <div class="cont-bloco">
                <img class="imagem-fundo" src="IMG/entrega.jpg" alt="">
            </div>            
        </div>
        <div class="cont-divisao">
            <div class="cont-bloco">
                <img class="numero-sobreposto" src="IMG/numeri-1.png" alt="">
                <img class="mulher-celular" src="IMG/mulher-celular.png" alt="">
            </div>
            <div class="cont-bloco">
                <h2>Como funciona?</h2>
                <h3>Faça seu cadastro </h3>
                <p>Uma vez que você fez seu cadastro, você pode iniciar cadastrando seus itens que podem ser emprestados.</p>
            </div>
        </div>
        <div class="cont-divisao">
            <div class="cont-bloco">
                <h3>Empreste</h3>
                <p>Com seu item cadastrado, você poderá escolher o destinatário e especificar a data de retorno. </p>
            </div>
            <div class="cont-bloco">
                <img class="numero-sobreposto" src="IMG/numero2.png" alt="">
                <img class="mulher-celular" src="IMG/caixa.png" alt="">
            </div>
        </div>
        <div class="cont-divisao">
            <div class="cont-bloco">
                <img class="numero-sobreposto" src="IMG/numero3.png" alt="">
                <img class="calendario" src="IMG/calendario.jpg" alt="">
            </div>
            <div class="cont-bloco">
                <h3>Organização</h3>
                <p>Emprestou algo? 
                    Nós mantemos seus registros organizados para que você sempre saiba com quem seus itens estão e quando retornarão para você.
                </p>
            </div>
        </div>
        <div class="cont-divisao"><button><a href="cadastro.php">Experimente agora</a></button></div>        
    </main>
</body>
</html>
